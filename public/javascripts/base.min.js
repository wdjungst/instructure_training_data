...
else var s=p/(2*Math.PI)*Math.asin(c/a);if(t<1)return-.5*(a*Math.pow(2,10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p))+b;return a*Math.pow(2,-10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p)*.5+c+b;},easeInBack:function(x,t,b,c,d,s){if(s==undefined)s=1.70158;return c*(t/=d)*t*((s+1)*t-s)+b;},easeOutBack:function(x,t,b,c,d,s){if(s==undefined)s=1.70158;return c*((t=t/d-1)*t*((s+1)*t+s)+1)+b;},easeInOutBack:function(x,t,b,c,d,s){if(s==undefined)s=1.70158;if((t/=d/2)<1)return c/2*(t*t*(((s*=(1.525))+1)*t-s))+b;return c/2*((t-=2)*t*(((s*=(1.525))+1)*t+s)+2)+b;},easeInBounce:function(x,t,b,c,d){return c-jQuery.easing.easeOutBounce(x,d-t,0,c,d)+b;},easeOutBounce:function(x,t,b,c,d){if((t/=d)<(1/2.75)){return c*(7.5625*t*t)+b;}else if(t<(2/2.75)){return c*(7.5625*(t-=(1.5/2.75))*t+.75)+b;}else if(t<(2.5/2.75)){return c*(7.5625*(t-=(2.25/2.75))*t+.9375)+b;}else{return c*(7.5625*(t-=(2.625/2.75))*t+.984375)+b;}},easeInOutBounce:function(x,t,b,c,d){if(t<d/2)return jQuery.easing.easeInBounce(x,t*2,0,c,d)*.5+b;return jQuery.easing.easeOutBounce(x,t*2-d,0,c,d)*.5+c*.5+b;}});(function(){$(function(){var all_users,arrayOfData,colors,getName,listItemClickHandler,plot,populate_data,usersClickHandler,week,weekClickHandler,weeks,_i,_len;arrayOfData=[];colors=['#FF3300','#FF6633','#33FF33','#0000CC','#00CCCC','#9966CC','#003300','#FF9933','#660033','#669900','#33CC33','#CC0000','#66FF66','#3399FF','#660066','#99FFFF','#99FF33','#FF6666','#00CC99','#FFFF99'];all_users='';getName=function(name){return alert(name);};populate_data=function(users,id){var $userDropdown,a,array,el,index,name,number,temp,_i,_len;arrayOfData=[];array=users.split('~');$userDropdown=$(id);$userDropdown.empty();for(index=_i=0,_len=array.length;_i<_len;index=++_i){el=array[index];a=el.split(',');name=a[1];if(typeof name!=="undefined"){$userDropdown.append("<li id='listItem"+index+"'><a href='#'>"+name+"</a></li>");}
number=parseFloat(a[0]);temp=[];temp.push(number);temp.push(a[1]);temp.push(colors[index]);arrayOfData.push(temp);}
arrayOfData.pop();$(".graph-container").empty();$(".graph-container").jqBarGraph({data:arrayOfData,legend:true,legendWidth:200,sort:'asc',width:'900px'});return listItemClickHandler();};weeks=['0','1','2','3','4','5','6','7','8'];weekClickHandler=function(week){return $("#week"+week).bind('click',function(e){$('.graph-container').empty();e.preventDefault();return $.get("/week/"+week,function(data){return populate_data(data,'#user_dropdown');});});};listItemClickHandler=function(){return $("#listItem0").bind('click',function(e){e.preventDefault();return alert('clicked');});};plot=$.jqplot("chartdiv",[[1,0,3,9,4,6,6,8]],{title:"Summary",axesDefaults:{labelRenderer:$.jqplot.CanvasAxisLabelRenderer,tickRenderer:$.jqplot.CanvasAxisTickRenderer,tickOptions:{angle:-30,fotSize:'10pt',mark:'cross'}},axes:{xaxis:{renderer:$.jqplot.CategoryAxisRenderer,ticks:['Week 1','Week 2','Week 3','Week 4','Week 5','Week 6','Week 7','Week 8'],pad:0},yaxis:{label:"Posts",min:0}},highlighter:{show:true,tooltipAxes:'y',sizeAdjust:7.5}});usersClickHandler=function(){return $('#modal-btn').bind('click',function(e){return $('#users-modal').modal('show');});};usersClickHandler();for(_i=0,_len=weeks.length;_i<_len;_i++){week=weeks[_i];weekClickHandler(week);}
$('#users-modal').on("shown",function(){plot.replot();return populate_data(all_users,'#modal_dropdown');});return $.get("/all_posts",function(response){all_users=response;return populate_data(response,'#user_dropdown');});});}).call(this);(function($){var opts=new Array;var level=new Array;$.fn.jqBarGraph=$.fn.jqbargraph=function(options){init=function(el){opts[el.id]=$.extend({},$.fn.jqBarGraph.defaults,options);$(el).css({'width':opts[el.id].width,'height':opts[el.id].height,'position':'relative','text-align':'center'});doGraph(el);};sum=function(ar){total=0;for(val in ar){total+=ar[val];}
